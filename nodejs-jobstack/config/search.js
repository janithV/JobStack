...
router.post('/search', function (req, res) {
  ...
  var sql = 'select id name from subjects where name like concat('%', @search, '%');';

  con.query(sql, [searchItem] ,function(err,rows){
...
//             for (var c = 0;
//                 c < a.length; c++)if (a[c] === b) return c;
//             return -1;
//         },
...
//                     for (this._callbacks = this._callbacks || {}, b = b.split(a);
//                         d = b.shift();)
...
//                     for (b = b.split(a);
//                         d = b.shift();)
//                         if (e = this._callbacks[d]) for (var f = 0; f < e.length; f += 1)e[f].call(this, { type: d, data: c });
...
//                     return b = b || c.noop, d = this.replace ? this.replace(this.url, g) : this.url.replace(this.wildcard, g), (e = i.get(d)) ? c.defer(function () { b(f.filter ? f.filter(e) : e) }) : this._get(d, b), !!e
//                 }
//             }), b;
//         }(),
...
//                     this.template = e(b.template, b.engine,
//                         this.valueKey),
//                     this.local = b.local,
...
//                             m.storage.set(g.protocol, c.getProtocol(), d.ttl)), m._mergeProcessedData(e);
//                     }

//                     var f, h, i, j, k, l, m = this, n = b + (d.thumbprint || "");
//                     return this.storage && (f = this.storage.get(g.thumbprint),
...
//                         i && j && !k ? (this._mergeProcessedData({ itemHash: i, adjacencyList: j }),
...
//                     return c.isString(a) ? (e.datum = {},
//                         e.datum[this.valueKey] = a) : e.datum = a,
//                         e.tokens = c.filter(e.tokens,
//                             function (a) {
...
//                     this.$hint = a(b.hint),
//                     this.$input = a(b.input).on("blur.tt", this._handleBlur).on("focus.tt", this._handleFocus).on("keydown.tt",
//                         this._handleSpecialKeyEvent), c.isMsie() ? this.$input.on("keydown.tt keypress.tt cut.tt paste.tt",
//                             function (a) {
...
//             function f(a, b) {
//                 return a = (a || "").replace(/^\s*/g, "").replace(/\s{2,}/g, " "),
...
//                     c ? this.trigger("whitespaceChanged", { value: this.query }) : b || this.trigger("queryChanged",
//                         { value: this.query = a })
//                 },
...
//                     var b, c, d, f;
//                     if (this.isVisible()) {
//                         if (b = this._getSuggestions(),
//                             c = b.filter(".tt-is-under-cursor"),
//                             c.removeClass("tt-is-under-cursor"),
//                             d = b.index(c) + a, d = (d + 1) % (b.length + 1) - 1, -1 === d)
//                             return this.trigger("cursorRemoved"),
//                                 void 0;
//                         -1 > d && (d = b.length - 1), f = b.eq(d).addClass("tt-is-under-cursor"),
...
//                         c = this.$menu.scrollTop(), d = a.position().top, e = d + a.outerHeight(!0);
...
//                         n = a("<div></div>").addClass(l).append(b.header).append(h).append(b.footer).appendTo(this.$menu)),
//                         d.length > 0 ? (this.isEmpty = !1, this.isOpen && this._show(),
...
//                                     }),
//                                     j.appendChild(k[0])
//                             }), n.show().find(".tt-suggestions").html(j)) : this.clearSuggestions(b.name),
...
//                     }), e.data("ttAttrs", { dir: e.attr("dir"), autocomplete: e.attr("autocomplete"), spellcheck: e.attr("spellcheck"), style: e.attr("style") }), e.addClass("tt-query").attr({ autocomplete: "off", spellcheck: !1 }).css(h.query); try { !e.attr("dir") && e.attr("dir", "auto") } catch (i) { } return e.wrap(c).parent().prepend(f).append(d)
//             }

//             function f(a) {
//                 var b = a.find(".tt-query"); c.each(b.data("ttAttrs"),
//                     function (a, d) {
...
//                 };
//             return c.isMsie() && c.mixin(h.query, { backgroundImage: "url(data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)" }),
//                 c.isMsie() && c.isMsie() <= 7 && (c.mixin(h.wrapper, { display: "inline", zoom: "1" }),
...
//                             h && i && !j && (a = this.inputView.getInputValue(),
//                                 b = a.replace(/\s{2,}/g, " ").replace(/^\s+/g, ""),
//                                 d = c.escapeRegExChars(b),
//                                 e = new RegExp("^(?:" + d + ")(.*$)", "i"),
//                                 f = e.exec(h), this.inputView.setHintValue(a + (f ? f[1] : "")))
//                         },
...
//                             d && (this.inputView.setInputValue(d.value),
...
//                                 c = "ltr" === this.inputView.getLanguageDirection() ? "leftKeyed" === a.type : "rightKeyed" === a.type, b && !c)) && (d = this.inputView.getQuery(),
//                                     e = this.inputView.getHintValue(), "" !== e && d !== e && (f = this.dropdownView.getFirstSuggestion(), this.inputView.setInputValue(f.value), this.eventBus.trigger("autocompleted", f.datum, f.dataset)))
//                         },
